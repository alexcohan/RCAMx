#' @export
READ_CAMx_2DMET_STATIC <- function(input,timezone="GMT",strp = TRUE, addutm = FALSE) {


HEAD <- .Fortran("READ_HEAD_BIG",ifile=input,NX=as.integer(1),NY=as.integer(1),NZ=as.integer(1),MAXS=as.integer(1),
...
MSPEC = MSPEC[MSPEC !="" & MSPEC !="\t" & MSPEC !="\001" & MSPEC !="azyeval"]
...
READ_CAMx_2DMET <- function(input,timezone="GMT",strp = TRUE, addutm = FALSE) {
dyn.load(Sys.getenv("FCAMx"))
#dyn.load("FCAMx/READ_CAMx.so")
library(dplyr)
library(tidyr)
